...
//----------------------------------------------------------------------------------------------------------
        ...
        foreach($students as $student){
            $absents = DB::table('absents')
                ->select('absents.*')
                ->where('absents.student_id','=',$student->id)
                ->whereYear('date', $year)
                ->whereMonth('date', $new)
                ->get();
            if($absents && $day == 1){
                $max = DB::table('counts')->where('student_id','=', $student->id)->max('date');
                $counts = DB::table('counts')
                        ->select('counts.remain')
                        ->where('counts.student_id','=',$student->id)
                        ->where('date', $max)
                        ->get();
...
//----------------------------------------------------------------------------------------------------------
        return view('admin.pages.home', compact('teachers','now','org','students','docs','guardians','centers','levels','materials','mats'));
    }
